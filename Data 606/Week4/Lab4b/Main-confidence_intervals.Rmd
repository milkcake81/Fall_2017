...
The sample distribution looks it's skewed to the right.  The typical size of the houses are from 1000 to 2000.  How I interpret typical is by taking the two bars with greatest area, which seems to be between 1000 and 2000.  

2.  Would you expect another student's distribution to be identical to yours? 
Would you expect it to be similar? Why or why not?

Not identical since it's a sample.  It may be similar since the sample size is 60, which is large enough to be a good reprentation of the population.  
...
Each observation must be independent of each other and the sample should be less than 10% of the population.
...
It means that if we were to get a parameter range with 2 standard error X number of times, 95 percent of parameter ranges will contain the actual parameter.  
...
Yes the confidence interval calculated above contains the mean of the population.
...
I would expect about 95% of the intervals to capture the true population mean.  Because we are looking within 1.96*se, which represents about 95%.  
...
Here, 3 intervals does not capture the population mean.  This is 85% so it does not match to the 95% confidence level. Probably increasing the number of samples taken will make it closer to 95% every single time.  

-   Pick a confidence level of your choosing, provided it is not 95%. What is 
    the appropriate critical value?
    
    I choose 50%.  Just for fun.
...
lower_vector_z <- samp_mean - z * samp_sd / sqrt(n) 
upper_vector_z <- samp_mean + z * samp_sd / sqrt(n)
plot_ci(lower_vector_z, upper_vector_z, mean(population))

```

There are 27 intervals that has population mean.  This turns out to be 54%.  This is slightly higher than 50% c.i. 
