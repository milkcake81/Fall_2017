...
This is mostly normal with very slight skewness to the right since the x-axis goes out to 6000.  
...
This looks normal.  The range of the data is much smaller than the population.  
...
In the sample, the means are rather similar between samp1 and samp2.  This is probably due to some chance.  Sample with 1000 should be closest to population mean. 
...
Sample_means50 has 5000 elements since it took 5000 different samples.  The sampling distribution is normal distribution with center being around 1500.  If we took 50,000 samples, I don't think the distribution will change much as it looks already quite normal. 
...
There are 100 elements.  Each element represent sample mean of the area with size of 50 in each.  
...
With the larger sample, the center remains approximately the same while the spread shrinks. 
...
The mean of the sample shown above will be my best point estimate.
...
I would guess from the histogram that the mean home price is 180,000.  The population mean is 180,796.  
...
150 sample has similar center as 50 sample, but it follows normal distribution a lot closer.  Based on 150 sample, I would still guess 180,000 to be the mean price of the home.  
...
150 sample has smaller spread.  I would definitely choose one with smaller spread.  
