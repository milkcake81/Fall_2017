...
Both male and female have normal disribution.  Male has a mean of 177.75cm with 7.18 sd.  Female has a mean of 164.87 with 6.54 sd. 
```{r Exercise 1}
library(ggplot2)
mhgt <- ggplot() + geom_histogram(data=mdims, aes(mdims$hgt), breaks=seq(130,200,by=5), col="blue") + labs(x='Height')
fhgt <- ggplot() + geom_histogram(data=fdims, aes(fdims$hgt), breaks=seq(130,200,by=5), col="red") + labs(x='Height')
library(cowplot)
plot_grid(mhgt, fhgt, labels=c('Male','Female'))
library(psych)
describe(mdims$hgt)
describe(fdims$hgt)
```
...
To me, the histogram follows closely to the normal distribution density graph.  
...
3.  Make a normal probability plot of `sim_norm`.  Do all of the points fall on 
    ...
    Not all points fall on the line.  The points deviate a bit between 2 and 3 SD.  This is actually similar to the real data's Q-Q plot.  
```{r Exercise 3}
library(data.table)
sim_graph <- qqnorm(sim_norm, col="red")
sim_graph <- data.table(cbind(sim_graph$x,sim_graph$y))
qqline(sim_norm)

real_graph <- qqnorm(fdims$hgt, col="blue")
real_graph <- data.table(cbind(real_graph$x,real_graph$y))
qqline(fdims$hgt)


ggplot() + geom_point(data=sim_graph, aes(x=sim_graph$V1, y=sim_graph$V2), col="red") + geom_point(data=real_graph, aes(x=real_graph$V1, y=real_graph$V2), col="blue")
```
...
4.  Does the normal probability plot for `fdims$hgt` look similar to the plots 
    ...
    I think it looks normal.  To my untrained eyes, the dots follow well to the qqline just as similar as those that are simulated.

5.  Using the same technique, determine whether or not female weights appear to 
    come from a normal distribution.
    
    Histogram shows slight skewness to the right.  Mean is 60.6 and SD is 9.62.
    QQplot shows some deviation from qqline from 1 to 3 SD.
    
    After comparing against simulated qqplot, it's clear that female weight is not normally distibuted with deviation at negative ends as well as positive ends of the qqplot from the qqline.  
```{r Exercise 5}
ggplot() + geom_histogram(data=fdims, aes(fdims$wgt), breaks=seq(20,150,by=5), col="blue") + labs(x='Weight')
fwgtstat <- describe(fdims$wgt)
qqnorm(fdims$wgt)
qqline(fdims$wgt)
set.seed(1014)
sim_norm_wgt <- rnorm(n = length(fdims$wgt), mean = fwgtstat$mean, sd = fwgtstat$sd)

sim_graph_wgt <- qqnorm(sim_norm_wgt, col="red")
sim_graph_wgt <- data.table(cbind(sim_graph_wgt$x,sim_graph_wgt$y))
qqline(sim_norm_wgt)

real_graph_wgt <- qqnorm(fdims$wgt, col="blue")
real_graph_wgt <- data.table(cbind(real_graph_wgt$x,real_graph_wgt$y))
qqline(fdims$wgt)


ggplot() + geom_point(data=sim_graph_wgt, aes(x=sim_graph_wgt$V1, y=sim_graph_wgt$V2), col="red") + geom_point(data=real_graph_wgt, aes(x=real_graph_wgt$V1, y=real_graph_wgt$V2), col="blue")
...
6.  Write out two probability questions that you would like to answer; one 
    ...
    What is the probability of a female between 150 to 165cm?
    What is the probablity of a female between 41kg to 52kg?
    
```{r Exercise 6}
fhtstat <- describe(fdims$hgt)
fsim_hts <- pnorm(q=165, mean=fhtstat$mean, sd=fhtstat$sd) - pnorm(q=150, mean=fhtstat$mean, sd=fhtstat$sd)
fsim_wts <- pnorm(q=52, mean=fwgtstat$mean, sd=fwgtstat$sd) - pnorm(q=41, mean=fwgtstat$mean, sd=fwgtstat$sd)
fdata_hts <- sum(fdims$hgt <= 165 & fdims$hgt >= 150) / length(fdims$hgt)
fdata_wts <- sum(fdims$wgt <= 52 & fdims$wgt >= 41) / length(fdims$wgt)

fdata_ideal <- sum(fdims$hgt <= 165 & fdims$hgt >= 150 & fdims$wgt <= 52 & fdims$wgt >= 41) / length(fdims$wgt)

print(c("Probabiltiy of Simulated Female height between 150 and 165cm is",fsim_hts))
print(c("Probabiltiy of Real Data Female height between 150 and 165cm is",fdata_hts))
print(c("Probabiltiy of Simulated Female weight between 41 and 52kg is",fsim_wts))
print(c("Probabiltiy of Real Data Female weight between 41 and 52kg is",fdata_wts))
print(c("Probabiltiy of Real Data Female that falls in both criteria is",fdata_ideal))
```

Height is much closer to the real data.  I apparently married a girl who falls within 13%, which must meet both criteria, if the sample data is a good representation of the population.  
...
```{r}
qqnorm(fdims$bii.di, main="Pelvic Diameter")
qqline(fdims$bii.di)
qqnorm(fdims$elb.di, main="Elbow Diameter")
qqline(fdims$elb.di)
qqnorm(bdims$age, main="age")
qqline(bdims$age)
qqnorm(fdims$che.de, main="Chest Depth")
qqline(fdims$che.de)
```

-   Note that normal probability plots C and D have a slight stepwise pattern.  
    Why do you think this is the case?
    Plot D: People surveyed are "healthy young adults", so most age group will be young with very few going to the old and also not many in the child age as well (but the age distribution range for child is much smaller than older group) So it's skiew right.  
    
    TO me, plot C doesn't really have stepwise pattern.  I think plot A does though
    
    Plot A: Chest depth probably has a lot to do breast size.  So some women have much bigger breast than others, so you get some outliers on the right.  And of course you get some people with no breast either due to too young age or genetically have small chest.  
...
-   As you can see, normal probability plots can be used both to assess 
    ...
    Skew to the right based on qq plot. Confirmed by histogram.  Sometimes, it's just easier using hist function.
    
```{r knee diamter normal probability}
qqnorm(fdims$kne.di)
qqline(fdims$kne.di)
