...
source("more/cdc.R")
setwd("C:\\Users\\soh1\\Box Sync\\CUNY\\Fall 2017\\Data 606\\Week1\\Lab1")
```
...
```
```{r}
print(c("interquntile for height = ",70-64))
print(c("interquantile for age =", 57-31))
table(cdc$gender)
table(cdc$exerany)
...
There are greater portion of males who smoke more than 100 cigarettes.  
...
I thought that people who smoke will have higher bmi, but the plot shows that bmi distribution is roughly the same for both smokers and non-smokers.
```{r}
...
Histogram with higher number of breaks is smoother and could tell at which bmi has the greatest concentration of people in more detail.
...
-   Make a scatterplot of weight versus desired weight. Describe the 
    ...
    There is direct relationship with people's current weight and desired weight.  Can observe from the graph that many poeple want to be lower than their current weight. 
...
-   What type of data is `wdiff`? If an observation `wdiff` is 0, what does 
    ...
    Wdiff id an integer and indicates the amount of weight that people wishes to lose or gain.  If wdiff is positive, the person wants to lose weight, if wdiff is negative, then the person wants to gain weight. IF wdiff is zero, the person's at ideal weight. 
...
-   Describe the distribution of `wdiff` in terms of its center, shape, and 
    ...
    People mostly wants to lose weight anyhwere from 10 to 15 lbs.   
...
-   Using numerical summaries and a side-by-side box plot, determine if men tend
    ...
    Women tend to want to lose more weight than men.  
...
-   Now it's time to get creative. Find the mean and standard deviation of 
    ...
    mean = 169.7 lbs
    sd = 40.1
    86% are within 1 standardard deviation
    
```{r}
mean(cdc$weight)
sd(cdc$weight)
length(cdc$weight[cdc$weight > (mean(cdc$weight) - sd(cdc$weight)) & mean(cdc$weight) < (cdc$weight + sd(cdc$weight))]) / 20000
```
